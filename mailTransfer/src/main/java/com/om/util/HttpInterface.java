...
public class HttpInterface {
	...
	public static String doPostJson(String url,JSONObject jsonParam)throws Exception {
    ...
    public static String doPostJson(String url,Map headMap ,String jsonParam)throws Exception {
        HttpClient httpclient = new DefaultHttpClient();
        HttpPost httppost = new HttpPost(url);
        String strResult = "";
        try {
            ...
            if(headMap!=null){
                ...
                while(it.hasNext()){
                    ...
                    String val = (String)headMap.get(key);
            ...
            log.debug("doPostJson:url:"+url);

            //httpclient.getParams().setParameter(CoreConnectionPNames.SO_TIMEOUT,300000);
            HttpResponse response = httpclient.execute(httppost);
            log.debug("doPostJson:response:"+response+"/"+response.getStatusLine().getStatusCode());
            if (response.getStatusLine().getStatusCode() == 200) {
                // 请求结束，返回结果
                ...
                String err = response.getStatusLine().getStatusCode() + "";
                String errInfo = EntityUtils.toString(response.getEntity(),"UTF-8");
                strResult += "发送失败:" + err+","+errInfo;
                throw new Exception(strResult);
            }
        } catch (Exception e) {
            log.error(e.getMessage(), e);
    ...
    public static String doPostJson(String url,Map headMap ,JSONObject jsonParam)throws Exception {
	    if(jsonParam!=null){
	        return doPostJson(url,headMap ,jsonParam.toJSONString());
        }else{
	        String s = null;
            return doPostJson(url,headMap ,s);
        }
    }

    public static Map doPostJsonV2(String url,Map headMap ,JSONObject jsonParam)throws Exception {
        String strResult = null;
        if(jsonParam!=null){
            strResult =  doPostJson(url,headMap ,jsonParam.toJSONString());
        }else{
            String s = null;
            strResult = doPostJson(url,headMap ,s);
        }
        log.debug("doPostJsonV2 reponse result:"+strResult);
        ...
        throw new Exception(m.get("msg").toString());
    ...
    public static String doPostJson(String url,Map headMap ,Map paramMap)throws Exception {
        HttpClient httpclient = new DefaultHttpClient();
        HttpPost httppost = new HttpPost(url);
        String strResult = "";
        try {
...
// 获取返回的数据
    ...
    public static String sendGet(String urlNameString,Map headMap) {
        String result = "";
        BufferedReader in = null;
        try {
