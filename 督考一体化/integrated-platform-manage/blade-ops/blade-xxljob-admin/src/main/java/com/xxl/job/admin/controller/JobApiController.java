...
public class JobApiController {
    ...
    public ReturnT<String> api(HttpServletRequest request, @PathVariable("uri") String uri, @RequestBody(required = false) String data) {
		...
		if ("callback".equals(uri)) {
			...
			return adminBiz.addJob(jobInfo);
		} else if ("start".equals(uri)) {
			int id = GsonTool.fromJson(data, Integer.class);
			return adminBiz.start(id);
		} else if ("remove".equals(uri)) {
			int id = GsonTool.fromJson(data, Integer.class);
			return adminBiz.remove(id);
		} else if ("stop".equals(uri)) {
			int id = GsonTool.fromJson(data, Integer.class);
			return adminBiz.stop(id);
		} else if ("update".equals(uri)) {
			XxlJobInfo jobInfo = GsonTool.fromJson(data, XxlJobInfo.class);
			return adminBiz.updateJob(jobInfo);
		} else if ("trigger".equals(uri)) {
			XxlJobInfo xxlJobInfo = GsonTool.fromJson(data, XxlJobInfo.class);
			return adminBiz.trigger(xxlJobInfo);
		} else {
